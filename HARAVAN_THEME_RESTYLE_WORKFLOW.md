...
- **Nạp skill `/haravan` TRƯỚC khi làm việc** (kiến thức nền platform Haravan) nếu agent có hệ skill; sau đó mới vào workflow này.
...
- **Kit là READ-ONLY khi chạy project**: mọi artifact/workaround/tool install lưu vào thư mục làm việc (`scratch/`, `<theme>/deliverables/`); bug kit/CLI → ghi improvement log, chỉ sửa kit trong phiên maintenance riêng có approval.
...
- **Template-first conversion khi có approved design**: dựng markup MỚI 100% theo design cho từng template trong scope; markup cũ không thể hiện được design thì xóa. Không đè CSS lên layout legacy, không giữ fallback/toggle/dual-path quay về layout cũ. Feature/behavior cũ được bảo tồn (bưng hook sang markup mới); đường lui layout cũ thì không.
- Layout mới từ design phải rebuild thành module sạch theo design 100%, không replace/overwrite nửa vời vào layout cũ.
- **CSS/JS mới viết vào file CÓ SẴN đúng template** (home → `index.*`, shell/global → `main.*`, blog → `blog.*`, article → `article.*`, template khác → cặp file riêng của nó). Không tạo file asset mới song song khi theme đã có slot; file mới = deviation phải log + duyệt.
- **Naming theo brand theme**: tên file/ảnh/class/settings key ship ra không được chứa tên design tool ("stitch"...); đặt theo ngữ nghĩa/brand của theme.
- **Asset jpg/png only — cấm webp**; ảnh generate/export phải nén (sharp q~80, đúng kích thước slot) trước khi vào `assets/`.
- **Đơn vị REM-first**: rem cho type/spacing/padding/gap (px ÷ 16), em chỉ cho padding nội bộ button/badge/icon-theo-text, px cho hairline/breakpoint. `html { font-size: 100% }`, cấm hack 62.5%.
- **Sửa thẳng file đích, không wrapper snippet dùng 1 chỗ**; snippet mới chỉ khi tái dùng 2+ chỗ hoặc theme đã có pattern tách sẵn.
...
- Ghi `SKILL_IMPROVEMENT_LOG.md` (trong `<theme>/deliverables/`) ngay khi gặp: kit/CLI bug, rule mù mờ, workaround, pitfall mới, hoặc BẤT KỲ yêu cầu thay đổi nào từ user — format `- [kit-bug|cli-gap|skill-gap|user-request|workaround|new-pitfall] chuyện gì → đề xuất sửa kit/skill`. Đây là nguồn nâng cấp kit/skill cho dự án sau.
...
## C. Workflow: Google Stitch / Template-First Conversion

> Bắt buộc đọc `STITCH_FIDELITY.md` trước khi implement. Mục đó là contract chống agent tự chế ra layout/section/copy/màu khác Stitch. Phạm vi là **template-first conversion** (§1A của contract): dựng markup mới theo design, không phải "visual layer" phủ lên layout cũ.

1. Dùng Stitch làm source of truth visual + structure; không ship raw Tailwind/CDN/placeholder trực tiếp. Khi authoring prompt Stitch, dùng `STITCH_PROMPT.template.md` với GLOBAL RULES (đủ mọi trang + modal + mobile, SHELL SPEC, MOTION SPEC).
...
- **Đè CSS lên markup legacy thay vì convert**: giữ DOM cũ rồi vá style "cho giống design" — phải dựng markup mới và xóa code bị thay.
- **Tạo file CSS/JS mới song song** thay vì extend `main`/`index`/`blog`/... sẵn có của theme.
- **Wrapper snippet dùng 1 chỗ** thay vì sửa thẳng file đích.
- **Tên file/ảnh/class dính "stitch"** hoặc tên generator trong code ship.
- **Asset webp** trong `assets/` hoặc tham chiếu webp trong CSS/Liquid.
- **Quên improvement log**: gặp kit bug/rule mù mờ/yêu cầu thay đổi từ user mà không ghi `SKILL_IMPROVEMENT_LOG.md`.
